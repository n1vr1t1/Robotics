...
namespace motion
{
    ...
    void DirectKinServer::computeDirectKinematics(const std::shared_ptr<custom_msg_interfaces::srv::ComputeDirKin::Request> request,
        ...
        try{
            ...
            // Populate response
            response->final_pose.position = {end_effector_position(0), end_effector_position(1), end_effector_position(2)};
    
            Eigen::Quaterniond quaternion(end_effector_orientation);
            response->final_pose.orientation = {quaternion.x(), quaternion.y(), quaternion.z(), quaternion.w()};
